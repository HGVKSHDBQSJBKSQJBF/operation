    ...
    def solution1(operation):
        ...
        def valider():
            ...
            if operation == cal.multiplication:
                ...
                label = Label(
                    erreur,
                    text="erreur de code veuillez contacter le support",
                    font=("Comic sans ms", 30),
                )
                label.place(x=0, y=0)
                sortie = Button(erreur, text="ok...", command=commands, width=15)
                sortie["font"] = f
        ...
        label1 = Label(
            calcul,
            text="entrer le deuxieme nombre (sert pas dans certaine operation)",
...
Label(fenetre_pseudo, text="Entrez votre pseudo :", font=("Comic sans ms", 20)).pack(
    pady=20
)
...
entry_pseudo.insert(0, pseudo)  # Remplir l'entrée avec le pseudo existant
...
pseudo = ""  # Variable globale pour stocker le pseudo de l'utilisateur
